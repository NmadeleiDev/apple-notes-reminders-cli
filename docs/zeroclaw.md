...
If the ZeroClaw host is Developer ID signed with the hardened runtime, its signing entitlements must include `com.apple.security.automation.apple-events = true`. The enclosing app's `Info.plist` must also declare `NSAppleEventsUsageDescription` and `NSRemindersFullAccessUsageDescription`. macOS attributes these child-process requests to the responsible host application, so declarations embedded only in this CLI are not sufficient for a hardened daemon host.
